        ...
        <p class="category">
          ...
          >
            <span class="transform">
              <span class="type">
                阅读至第{{ Number(ReadHistoryItem.historyChapter) + 1 }}章</span
              >
            </span>
          </template>
...
export default {
  ...
  created() {
    ...
    this.axios
      .get("http://novel.kele8.cn/book-info/" + this.book._id + "")
      .then((response) => {
  ...
  methods: {
    ...
    //   console.log(e);
    // },
    continueToReadMy(e, chapters,index) {
      // console.log(e);
      // this.$toast("加载中,请稍等...");
      console.log("继续阅读:历史", e, chapters,this.resultBookChapter,index);
    ...
    continueToReadHistory(id, chapters, historyChapter) {
      console.log("继续阅读:历史ID", id);
      console.log("继续阅读:历史章节列表", chapters);
      console.log("继续阅读:当前章节索引", historyChapter);
      console.log("");
      console.log(this.resultHistoryBookChapter);
      ...
      this.$router.push({
        path: "/book/" + id + "/read",
        query: { readIndex: historyChapter },
      });
    },
